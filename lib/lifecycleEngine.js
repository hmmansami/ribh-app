...
async function handleAbandonedCart(store, data) {
    ...
    // Log the offer (actual email sending handled by existing system)
    console.log(`📧 Recovery offer ready for ${email}: ${offer.headline}`);
...
async function handleOrderCreated(store, data) {
    ...
    console.log(`📧 Upsell offer ready for ${email}: ${offer.headline}`);
    return offer;
}
